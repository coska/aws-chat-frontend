...
const ServerSentEvents = () => {
    ...
    useEffect(() => {
        const sseForLastMessage = new EventSource(`${serverURL}/sse-server/user/messages?name=${user}`, {
            withCredentials: true,
        });

        const sseForAllMessages = new EventSource(`${serverURL}/sse-server/user/messages/all?name=${user}`, {
            withCredentials: true,
        });

        const sseForUsers = new EventSource(`${serverURL}/sse-server/users?name=${user}`, {
            withCredentials: true,
        });
    ...
    const handleSubmit = async (event) => {
        ...
        if (nameInputValue && nameInputValue.trim() !== "") {
            ...
            const response = await fetch(`${serverURL}/sse-server/user?name=${nameInputValue}`, requestOptions);
            ...
            {user && users && users.length > 0 ? (
                <div className="chat-boxes">
                    {users.map((chatUser, index) => (
                        <ChatBox key={index} index={index} divRefs={divRefs} messages={messages} user={user} chatUser={chatUser} />
                    ))}
                </div>
            ) : user ? (
